...
#' @importFrom magclass as.magpie add_dimension getSets
#' getNames dimSums mbind getRegions
...
producerPrices <- function(gdx, ...) {
  prices <- as.magpie(readGDX(gdx = gdx, name = "PPX0")[["data"]],
                      spatial = "cty",
                      temporal = "yrs")
  production <- as.magpie(readGDX(gdx = gdx, name = "QSX0")[["data"]],
                          spatial = "cty",
                          temporal = "yrs")

  # At this stage, the unit is 000 2005 USD but later will be
  # divided by 000 mt so okay to keep unit as 2005 USD per mt here
