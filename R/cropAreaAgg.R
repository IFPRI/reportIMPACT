...
cropAreaAgg <- function(gdx){
  ...
  fpath <- system.file("extdata", file = "mapping_items.xlsx", package = "DOORMAT")

  group_disagg_mapping <- read_xlsx(path = fpath,
                                    sheet = "Aggregation Crops",
                                    range = "B8:D150",
                                    .name_repair = "unique_quiet")
  colnames(group_disagg_mapping) <- tolower(gsub(pattern = " |-",replacement = "_",x = colnames(group_disagg_mapping)))
  group_disagg_mapping <- unique(group_disagg_mapping)

  df <- df2 <- merge(df,group_disagg_mapping)
