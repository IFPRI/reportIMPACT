...
#' @param bind If new aggregation should be binded to the original dataframe passed on to "df" argument
#'
...
levelSum <- function(df, dim_name = NULL, spatial = "region", temporal = "yrs", na.rm = TRUE, bind = TRUE){

  df_mag <- as.magpie(df, spatial = spatial, temporal = temporal)

  dim_to_sum <- as.numeric(gsub(pattern = "d",
                                replacement = "",
                                x = names(getSets(df_mag)[getSets(df_mag) == dim_name])))
  ...
  out[,dim_name] <- gsub(pattern = new_name,replacement = NA,x = out[,dim_name])
