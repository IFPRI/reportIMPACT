...
#'
#' @return GLO DomesticImportPrices
#' @param ... Arguments to aggregateIMPACT call. See ?aggregateIMPACT
#' @importFrom DOORMAT readGDX aggregateIMPACT
#' @importFrom magclass as.magpie getSets getNames dimSums getItems
#' @author Abhijeet Mishra
#' @examples
#' \dontrun{x <- DomesticImportPrices(gdx)}
#' @export

DomesticImportPrices <- function(gdx, ...) {

  prices <- as.magpie(readGDX(gdx = gdx, name = "PMX0")[["data"]],
                      spatial = "cty", temporal = "yrs")
  demand <- as.magpie(readGDX(gdx = gdx, name = "QMX0")[["data"]],
                      spatial = "cty", temporal = "yrs")
  ...
  demand_aggregate <- aggregateIMPACT(df = readGDX(gdx = gdx, name = "QMX0"),
                                      ...)
