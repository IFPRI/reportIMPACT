...
consumerPrices <- function(gdx){
  prices <- as.magpie(readGDX(gdx = gdx,name = "PCX0")[["data"]],spatial="cty",temporal="yrs")
  ...
  price_aggregate <- as.magpie(value_aggregate)/collapseNames(as.magpie(demand_aggregate))
